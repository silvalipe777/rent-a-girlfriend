...
export async function POST() {
  try {
    ...
    return NextResponse.json({ message: "10个加密AI助手播种成功！" });
  } catch (error) {
    const message = error instanceof Error ? error.message : "数据库播种失败";
